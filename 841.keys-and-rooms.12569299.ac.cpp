...
public:
    bool canVisitAllRooms(vector<vector<int>>& rooms) {
        ...
        return visitedroom_.size() == rooms.size();
    }

private:
    set<int> visitedroom_;
    void visit(vector<vector<int>>& rooms, int key)
    {
        if (visitedroom_.count(key) == 0)
        {
            visitedroom_.insert(key);
            auto keys = rooms[key];
            for (auto key : keys)
            {
                visit(rooms, key);
