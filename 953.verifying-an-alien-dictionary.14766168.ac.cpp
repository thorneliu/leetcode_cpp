...
public:
    bool isAlienSorted(vector<string>& words, string order) {
        ...
        {
            map_.insert({order[i], (i + 'a')});
        }

        vector<string> converted;
        for (auto word : words)
        {
            converted.push_back(convert(map_, word));
        }

        for (int i = 1; i < converted.size(); i++)
        {
            if (converted[i - 1] >= converted[i]) return false;
        }

        return true;
    }
private:
    string convert(unordered_map<char, char>& map_, const string& word)
    {
        stringstream oss;
        for (auto c : word)
        {
