/*
 * @lc app=leetcode.cn id=807 lang=c
 *
 * [807] 自定义字符串排序
 *
...
int maxIncreaseKeepingSkyline(int** grid, int gridRowSize, int *gridColSizes) {
    int rowSkyline[MAXLEN] = {0};
    int colSkyline[MAXLEN] = {0};
    ...
    for (int i = 0; i < gridRowSize; i++)
    {
        ...
        {
            int value = grid[i][j];
            if (value > rowSkyline[i]) rowSkyline[i] = value;
            if (value > colSkyline[j]) colSkyline[j] = value;
        }
    }
    ...
    for (int i = 0; i < gridRowSize; i++)
    {
        for(int j = 0; j < gridRowSize; j++)
        {
            result += min(rowSkyline[i], colSkyline[j]) - grid[i][j];
        }
    }
    
    *gridColSizes = gridRowSize;
    return result;
}
